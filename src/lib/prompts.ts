...
export function buildPrompt(kind: TabKind, trip: Trip): { system: string; user: string } {
  ...
  switch (kind) {
    ...
    case "route": {
      ...
      return {
        system:
          `You are an expert travel route optimizer. ${sharedAssumptions} The traveler wants to visit all of these destinations: ${list}. Trip dates: ${start} to ${end}. Total budget: ${trip.budget} ${trip.currency}.\n\nDo the following:\n\n1. Determine the single most efficient order to visit all destinations, optimizing for lowest total travel cost and shortest total travel time. Do NOT assume the order the user listed is correct — rearrange freely based purely on geography, travel cost, and travel time.\n2. Present the recommended route clearly as: Destination A → Destination B → Destination C.\n3. For each leg between destinations, provide: transport mode (flight, train, bus or ferry), estimated travel time, estimated cost in ${trip.currency}, best booking tip, and best time to book.\n4. Provide a total estimated inter-destination transport cost and what percentage of the overall budget that represents.\n5. If an alternative route is within 10% of the same cost but significantly faster (or vice versa), mention it briefly as an alternative option.\n\nFormat as Markdown. Start with a bold "**Recommended Route:** ..." line listing the ordered destinations. Then a bold heading for each leg (e.g. **Leg 1: City A → City B**). End with a "## Total Transport Budget" section and, if applicable, a "## Alternative Option" section.`,
        user: ctx,
      };
    }
  }
